package com.aqsl.teamupfp.View.InputAbssen
...
import com.aqsl.teamupfp.BuildConfig
import com.aqsl.teamupfp.R
import com.aqsl.teamupfp.Utils.BitmapManager.bitmapToBase64
import com.aqsl.teamupfp.ViewModel.AbsenViewModel
...
import kotlinx.android.synthetic.main.activity_isi_absen.*
import java.io.File
...
class IsiAbsenActivity : AppCompatActivity() {
    ...
    lateinit var exifInterface: androidx.exifinterface.media.ExifInterface
        ...
        fusedLocationClient.lastLocation
            .addOnSuccessListener(this) { location ->
                ...
                if (location != null) {
                    ...
                    val geocoder = Geocoder(this@IsiAbsenActivity, Locale.getDefault())
                    try {
                        val addresList =
                            geocoder.getFromLocation(strCurrentLatitude, strCurrentLongitude, 1)
                        if (addresList != null && addresList.size > 0) {
                            strCurrentLocation = addresList[0].getAddressLine(0)
                    ...
                    progressDialog.dismiss()
                    Toast.makeText(
                        this@IsiAbsenActivity,
                        "Gagal mendapat lokasi , Silahkan periksa jaringan atau GPS Anda",
                        Toast.LENGTH_SHORT
                    ).show()
                    strLatitude = "0"
                    strLongitude = "0"
    ...
    private fun setInitLayout() {
        ...
        inputTanggal.setOnClickListener {
            ...
            val date =
                DatePickerDialog.OnDateSetListener { _: DatePicker, year: Int, monthofYear: Int, dayOfonth: Int ->
                    tanggalAbsen[Calendar.YEAR] = year
                    tanggalAbsen[Calendar.MONTH] = monthofYear
                    tanggalAbsen[Calendar.DAY_OF_MONTH] = dayOfonth
                    val strFormatDefault = "dd MMM yyy HH:mm"
                    val simpleDateFormat = SimpleDateFormat(strFormatDefault, Locale.getDefault())
                    inputTanggal.setText(simpleDateFormat.format(tanggalAbsen.time))
                }
            DatePickerDialog(
                this@IsiAbsenActivity, date,
        ...
        layoutImage.setOnClickListener {
            Dexter.withContext(this@IsiAbsenActivity)
                .withPermissions(
    ...
    private fun setUploadData() {
        btnAbsen.setOnClickListener {
            ...
            if (strFilePath.equals(null) || strNama.isEmpty() || strCurrentLocation.isEmpty()
                ...
                Toast.makeText(
                    this@IsiAbsenActivity,
                    "Laporan anda berhasil dikirim!", Toast.LENGTH_SHORT
                ).show()
                finish()
            }
        }
    ...
    private fun convertImage(imageFilePath: String?) {
        ...
        if (imageFile.exists()){
            ...
            val orientation = exifInterface.getAttributeInt(androidx.exifinterface.media.ExifInterface.TAG_ORIENTATION, 0)
            val matrix = Matrix()
            if (orientation == 6) {
            ...
            if (bitmapImage == null) {
                Toast.makeText(this@IsiAbsenActivity,
                    "Foto anda belum ada", Toast.LENGTH_LONG).show()
            } else {

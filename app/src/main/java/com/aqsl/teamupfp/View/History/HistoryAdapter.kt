...
import com.aqsl.teamupfp.Utils.BitmapManager.base64ToBitmap
import com.bumptech.glide.load.engine.DiskCacheStrategy
import com.google.android.material.imageview.ShapeableImageView

import kotlinx.android.synthetic.main.activity_beranda.view.*
import kotlinx.android.synthetic.main.activity_list_history.view.*
import java.lang.String

class HistoryAdapter(
    var mContext: Context,
    var modelDatabase: MutableList<ModelDb>,
    var mAdapterCallback: HistoryAdapterCallback
) : RecyclerView.Adapter<HistoryAdapter.ViewHolder>() {

    fun setDataAdapter(items: List<ModelDb>) {
        modelDatabase.clear()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.activity_list_history, parent, false)
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var cvHistory: CardView = itemView.cvHistorylist
        var imageProfile: ShapeableImageView = itemView.imageProfile
        var colorStatus: View = itemView.colorStatus

        init {
            cvHistory.setOnClickListener {
                val modelAbsen = modelDatabase[adapterPosition]
                mAdapterCallback.onDelete(modelAbsen)
            }
        }
    }

    interface HistoryAdapterCallback {
        fun onDelete(modelDatabase: ModelDb?)
    }
}
